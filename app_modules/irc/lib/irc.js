...
Client.prototype.who = function(channel, callback) { // {{{
    if ( typeof callback === 'function' ) {
        var callbackWrapper = function(info) {
            if ( (info && info.channel == channel&&channel&&channel.indexOf(" ")==-1) || (channel &&channel.indexOf(" ")!=-1 && this._whoNick.indexOf(channel.split(" ")[0])==-1))   {
    ...
    if (channel.indexOf(" ")!=-1){
       this._whoNick.push(channel.split(" ")[0]);
        this.send('WHO', channel.split(" ")[0],channel.split(" ")[1]);
    }else{
...
Client.prototype._addWhoData = function(channel, nick, value, onlyIfExists) { // {{{
    ...
    if (this._whoNick.indexOf(nick)==-1){
    this._whoData[channel]['nicks'][nick] = data;
    }
    else{
       data.nick=nick;
        this._whoData[nick]=data;
        }
}
Client.prototype._clearWhoData = function(channel) { // {{{
    ...
    return data;
        ...
        var data = this._whoData[channel];
         delete this._whoData[channel];
          this._whoNick.splice(this._whoNick.indexOf(channel),1);
            return data
